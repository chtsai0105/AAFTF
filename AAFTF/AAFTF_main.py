...
def run_subtool(parser, args):
    ...
    if command == "trim":
        ...
        import AAFTF.check_dependencies as submodule
    else:
        parser.parse_args("")
        return
...
def main():
    ...
    parser.add_argument("-q", "--quiet", help="Do not output warnings to stderr", action="store_true", dest="quiet")
    ...
    parser_trim.add_argument(
        "-o", "--out", type=str, required=False, dest="basename", help="Output basename, default to base name of --left reads"
    )
    ...
    parser_trim.add_argument(
        "-ml", "--minlen", type=int, default=75, required=False, help="Minimum read length after trimming"
    )
    ...
    parser_trim.add_argument("-r", "--right", type=str, required=False, help="right/reverse reads of paired-end FASTQ.")

    parser_trim.add_argument(
        "-aq", "--avgqual", type=int, default=10, required=False, help="Average Quality of reads must be > than this"
    )
    ...
    parser_trim.add_argument("-v", "--debug", action="store_true", help="Provide debugging messages")

    parser_trim.add_argument("--pipe", action="store_true", help="AAFTF is running in pipeline mode")

    parser_trim.add_argument(
        "--method", default="bbduk", choices=["bbduk", "trimmomatic", "fastp"], help="Program to use for adapter trimming"
    )

    parser_trim.add_argument("-m", "--memory", type=int, dest="memory", required=False, help="Max Memory (in GB)")

    parser_trim.add_argument("--merge", action="store_true", help="Merge paired end reads when running fastp")

    parser_trim.add_argument("--prefix", type=str, required=False, help="Prefix for outfiles")
    ...
    trimmomatic_group.add_argument(
        "--trimmomatic_adaptors", default="TruSeq3-PE.fa", help="Trimmomatic adaptor file"
    )
    ...
    parser_mito.add_argument("-l", "--left", required=True, help="Left (Forward) reads")

    parser_mito.add_argument("-r", "--right", required=True, help="Right (Reverse) reads")

    parser_mito.add_argument("-o", "--out", type=str, required=True, help="Output FASTA file for mitochondrial genome")

    parser_mito.add_argument("--minlen", default=10000, type=int, help="Minimum expected genome size")

    parser_mito.add_argument("--maxlen", default=100000, type=int, help="Maximum expected genome size")

    parser_mito.add_argument(
        "-s", "--seed", required=False, help="Seed sequence, ie related mitochondrial genome." + "default: A. nidulans"
    )

    parser_mito.add_argument("--starting", required=False, help="FASTA file of start sequence, rotate genome to, default COB")
    ...
    parser_filter.add_argument("-w", "--workdir", type=str, help="temp directory")

    parser_filter.add_argument(
        "-c", "--cpus", type=int, metavar="cpus", required=False, default=1, help="Number of CPUs/threads to use."
    )

    parser_filter.add_argument("-o", "--out", dest="basename", type=str, required=False, help="Output basename")

    parser_filter.add_argument(
        "-v", "--debug", action="store_true", help="Provide debugging messages and do not remove contamdb " + "matching BAM file"
    )
    ...
    parser_filter.add_argument("-u", "--screen_urls", type=str, nargs="*", help="URLs to download and screen out initial reads.")

    parser_filter.add_argument(
        "-s", "--screen_local", type=str, nargs="+", help="Local FASTA file(s) to use contamination screen"
    )

    parser_filter.add_argument("-l", "--left", required=True, help="Left (Forward) reads")

    parser_filter.add_argument("-r", "--right", required=False, help="Right (Reverse) reads")

    parser_filter.add_argument("--AAFTF_DB", type=str, required=False, help="Path to AAFTF resources, defaults to $AAFTF_DB")
    ...
    parser_filter.add_argument("-m", "--memory", type=int, dest="memory", required=False, help="Max Memory (in GB)")

    parser_filter.add_argument("--pipe", action="store_true", help="AAFTF is running in pipeline mode")
    ...
    parser_asm.add_argument("-w", "--workdir", type=str, dest="workdir", help="assembly output directory")
    ...
    parser_asm.add_argument("-l", "--left", required=False, help="Left (Forward) reads")

    parser_asm.add_argument("-r", "--right", required=False, help="Right (Reverse) reads")
    parser_asm.add_argument("-lr", "--longreads", required=False, help="Long Read fastq (pacbio or ONT)")
    parser_asm.add_argument(
    ...
    parser_asm.add_argument(
        "--isolate", required=False, action="store_true", dest="isolate", help="Run in isolate mode (default)"
    )

    parser_asm.add_argument("--no-isolate", required=False, action="store_false", dest="isolate", help="Don't run --isolate mode")

    parser_asm.add_argument("-v", "--debug", action="store_true", help="Print Spades stdout to terminal")
    ...
    parser_asm.add_argument("--pipe", action="store_true", help="AAFTF is running in pipeline mode")
    ...
    parser_vecscreen.add_argument("-c", "--cpus", type=int, metavar="cpus", default=1, help="Number of CPUs/threads to use.")

    parser_vecscreen.add_argument(
        "-i", "--input", "--infile", type=str, required=True, dest="infile", help="Input contigs or scaffold assembly"
    )

    parser_vecscreen.add_argument("-o", "--outfile", type=str, required=True, help="Output vector screened and cleaned assembly")

    parser_vecscreen.add_argument(
        "-pid", "--percent_id", type=int, required=False, help="Percent Identity cutoff for vecscreen adaptor matches"
    )

    parser_vecscreen.add_argument("--prefix", type=str, required=False, help="Prefix for tempfiles")

    parser_vecscreen.add_argument(
        "-w", "--workdir", "--tmpdir", type=str, help="Working directory to store datafiles and processes in"
    )

    parser_vecscreen.add_argument("--AAFTF_DB", type=str, required=False, help="Path to AAFTF resources, defaults to $AAFTF_DB")

    parser_vecscreen.add_argument(
        "-s", "--stringency", default="high", choices=["high", "low"], help="Stringency to filter VecScreen hits"
    )

    parser_vecscreen.add_argument("-v", "--debug", action="store_true", dest="debug", help="Provide debugging messages")

    parser_vecscreen.add_argument("--pipe", action="store_true", help="AAFTF is running in pipeline mode")
    ...
    parser_fcs_screen.add_argument("-o", "--outfile", type=str, required=True, help="Output vector screened and cleaned assembly")
    ...
    parser_fcs_screen.add_argument(
        "--image", type=str, required=False, help="Container file (or will download and look in AAFTF_DB)"
    )
    ...
    parser_fcs_screen.add_argument("--AAFTF_DB", type=str, required=False, help="Path to AAFTF resources, defaults to $AAFTF_DB")

    parser_fcs_screen.add_argument("--prok", action="store_true", help="Run in Prokaryote matching mode")

    parser_fcs_screen.add_argument("--euk", action="store_true", help="Run in Eukaryote matching mode (Default)")

    parser_fcs_screen.add_argument(
        "--fcs_script", type=str, required=False, help="location of the run_fcsadaptor.sh script (or will download automatically)"
    )

    parser_fcs_screen.add_argument("-v", "--debug", action="store_true", dest="debug", help="Provide debugging messages")

    parser_fcs_screen.add_argument("--pipe", action="store_true", help="AAFTF is running in pipeline mode")
    ...
    parser_fcsgx.add_argument("-i", "--input", type=str, required=True, help="Input contigs or scaffold assembly")
    ...
    parser_fcsgx.add_argument("--prefix", type=str, required=False, help="Prefix for tempfiles")
    ...
    parser_fcsgx.add_argument("-d", "--db", required=False, default="/my_tmpfs/gxdb/all", help="gxdb database path")

    parser_fcsgx.add_argument("-c", "--cpus", type=int, metavar="cpus", default=1, help="Number of CPUs/threads to use.")
    ...
    parser_fcsgx.add_argument("-v", "--debug", action="store_true", dest="debug", help="Provide debugging messages")

    parser_fcsgx.add_argument("--AAFTF_DB", type=str, required=False, help="Path to AAFTF resources, defaults to $AAFTF_DB")

    parser_fcsgx.add_argument("--pipe", action="store_true", help="AAFTF is running in pipeline mode")
    ...
    parser_sour.add_argument("-i", "--input", type=str, required=True, help="Input contigs or scaffold assembly")
    ...
    parser_sour.add_argument("--prefix", type=str, required=False, help="Prefix for tempfiles")

    parser_sour.add_argument("-l", "--left", required=False, help="Left (Forward) reads")

    parser_sour.add_argument("-r", "--right", required=False, help="Right (Reverse) reads")

    parser_sour.add_argument("-p", "--phylum", required=True, nargs="+", help="Phylum or Phyla to keep matches, i.e. Ascomycota")

    parser_sour.add_argument("--sourdb", required=False, help="SourMash LCA taxonomy database (defaults to k-31)")

    parser_sour.add_argument(
        "-k", "--kmer", required=False, default="31", help="SourMash LCA kmersize when taxonomy database was built"
    )

    parser_sour.add_argument("-mc", "--mincovpct", default=5, type=int, help="Minimum percent of N50 coverage to remove")

    parser_sour.add_argument("-c", "--cpus", type=int, metavar="cpus", default=1, help="Number of CPUs/threads to use.")
    ...
    parser_sour.add_argument("-v", "--debug", action="store_true", dest="debug", help="Provide debugging messages")
    ...
    parser_sour.add_argument("--AAFTF_DB", type=str, required=False, help="Path to AAFTF resources, defaults to $AAFTF_DB")

    parser_sour.add_argument(
        "--just-show-taxonomy", dest="taxonomy", action="store_true", help="Show taxonomy information and exit"
    )

    parser_sour.add_argument("--pipe", action="store_true", help="AAFTF is running in pipeline mode")
    ...
    parser_rmdup.add_argument("-i", "--input", type=str, required=True, help="Input Assembly fasta file(contigs or scaffolds)")
    ...
    parser_polish.add_argument(
        "-o", "--out", "--outfile", type=str, dest="outfile", required=False, help="Output a Polished assembly"
    )
    ...
    parser_polish.add_argument("-c", "--cpus", type=int, metavar="cpus", default=1, help="Number of CPUs/threads to use.")

    parser_polish.add_argument(
        "-m", "--memory", type=int, default=16, dest="memory", required=False, help="Max Memory (in GB)"
    )

    parser_polish.add_argument("-v", "--debug", action="store_true", help="Provide debugging messages")

    parser_polish.add_argument(
        "-it", "--iterations", type=int, default=5, help="Number of Polishing iterations to run"
    )
    parser_polish.add_argument(
        "--method",
        type=str,
    ...
    parser_polish.add_argument(
        ...
        type=str,
        required=False,
        help="The name of the left/forward Illumina reads of paired-end " + "FASTQ formatted reads.",
    )
    ...
    parser_polish.add_argument(
        ...
        type=str,
        required=False,
        help="The name of the right/reverse Illumina reads of paired-end " + "FASTQ formatted reads.",
    )

    parser_polish.add_argument("-lr", "--longreads", required=False, help="Long Read FASTQ (PacBio or ONT)")
    parser_polish.add_argument(
        "-w",
        "--workdir",
        "--tmpdir",
    ...
    parser_polish.add_argument("--prefix", type=str, required=False, help="Prefix for readfiles")

    parser_polish.add_argument(
        "--diploid", action="store_true", help="Run pilon in diploid mode - affects heterozygous SNP calling"
    )
    ...
    parser_polish.add_argument("--pipe", action="store_true", help="AAFTF is running in pipeline mode")
    ...
    parser_sort.add_argument("-i", "--input", "--infile", required=True, dest="input", help="Input genome assembly FASTA")

    parser_sort.add_argument("-o", "--out", "--output", required=True, dest="out", help="Output genome assembly FASTA")
    ...
    parser_sort.add_argument(
        "-n", "--name", "--basename", default="scaffold", dest="name", help="Basename to rename FASTA headers"
    )
    ...
    parser_assess.add_argument(
        "-t", "--telomere_monomer", type=str, help="Telomere repeat monomer to search for.", default="TAA[C]+"
    )

    parser_assess.add_argument(
        "-n", "--telomere_n_repeat", type=int, default=2, help="Telomere minimum number of monomer repeats."
    )
    ...
    parser_pipeline.add_argument("-r", "--right", type=str, required=False, help="right/reverse reads of paired-end FASTQ.")
    ...
    parser_pipeline.add_argument(
        "-u", "--screen_urls", type=str, nargs="*", help="URLs to download and screen out initial reads."
    )

    parser_pipeline.add_argument("-it", "--iterations", type=int, default=5, help="Number of Pilon Polishing iterations to run")

    parser_pipeline.add_argument(
        "-mc", "--mincontiglen", type=int, default=500, required=False, help="Minimum length of contigs to keep"
    )

    parser_pipeline.add_argument("--AAFTF_DB", type=str, required=False, help="Path to AAFTF resources, defaults to $AAFTF_DB")

    parser_pipeline.add_argument("-w", "--workdir", type=str, help="temp directory")

    parser_pipeline.add_argument("-v", "--debug", action="store_true", help="Provide debugging messages")

    parser_pipeline.add_argument(
        "-p", "--phylum", required=True, nargs="+", help="Phylum or Phyla to keep matches, i.e. Ascomycota"
    )
    ...
    parser_check_deps.add_argument("-v", "--debug", action="store_true", help="Provide debugging messages")
    parser_check_deps.add_argument("--pipe", action="store_true", help="AAFTF is running in pipeline mode")
